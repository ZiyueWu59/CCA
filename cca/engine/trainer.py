...
from cca.utils.comm import get_world_size, synchronize
from cca.utils.metric_logger import MetricLogger
...
def do_train(
    ...
    logger = logging.getLogger("cca.trainer")
    ...
    for epoch in range(arguments["epoch"], max_epoch + 1):
        ...
        for iteration, (batches, targets, attri_input_emb, _) in enumerate(data_loader):
            iteration += 1
            data_time = time.time() - end

            batches = batches.to(device)
            targets = targets.to(device)
            attri_input_emb = attri_input_emb.to(device)

            def closure():
                optimizer.zero_grad()
                loss = model(batches, attri_input_emb, targets)
