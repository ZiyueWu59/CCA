...
import torch
from cca.data.datasets.evaluation import evaluate
...
def compute_on_dataset(model, data_loader, device, timer=None):
    ...
    for batch in tqdm(data_loader):
        batches, targets, attri_input_emb, idxs = batch
        with torch.no_grad():
            if timer:
                timer.tic()
            output = model(batches.to(device), attri_input_emb.to(device))
            if timer:
...
def _accumulate_predictions_from_multiple_gpus(predictions_per_gpu):
    ...
    if len(idxs) != idxs[-1] + 1:
        logger = logging.getLogger("cca.inference")
        logger.warning(
